...
def learning_rate_with_decay(                                                
  ...
  def poly_rate_fn(global_step):
    ...
    wrate = (plr * tf.cast(global_step, tf.float32) / tf.cast(    #plr
        w_steps, tf.float32))
...
def resnet_model_fn(features, labels, mode, model_class,                      # resnet model function
  ...
  model = model_class(resnet_size, data_format, resnet_version=resnet_version,     # model class
                      dtype=dtype)

  logits,embedding = model(features, mode == tf.estimator.ModeKeys.TRAIN)                 
  ...
  if label_smoothing != 0.0:                                                  
    ...
    cross_entropy = tf.compat.v1.losses.sparse_softmax_cross_entropy(           # sparse softmax
